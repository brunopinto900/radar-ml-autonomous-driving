...
- **Raw feature profile on the real errors** (`real_model_feature_profile`): `rcs` shows the classic overlap pattern in every regime, including sparse, misclassified car (median 9.06) sits at car's own high tail (its own p75 is 11.4) reaching into large_vehicle's typical value, not large_vehicle's low tail. The sparse regime's own `large_vehicle_true` bucket here (median 2.69) looked like the reference to check against, but it's val-split-restricted (n=1123) and is itself a split-composition artifact, `large_vehicle`'s true sparse-regime rcs, confirmed independently three ways (whole-dataset population, an isolated sparse-only probe, and a partial-dependence sweep on the real model, `sparse_rcs_mechanism_check.py`), is actually higher than car's, not lower. Lesson: the 6-fold split-sensitivity check exists for metrics, this shows the same caution applies to raw diagnostic statistics on a small, regime-conditioned subset, check against the whole dataset (or multiple splits) before trusting a single val-restricted number as ground truth. `x_extent` (length) of misclassified cars sits close to car_correct's own value in every regime, not large_vehicle's, length overlap does not explain the confusion despite `x_rel` topping the importance ranking. `y_extent` (width) and `spatial_extent` (orientation-robust diagonal) tell the real story: misclassified cars are anomalously wide/large relative to their own class, overshooting even large_vehicle's typical value in sparse/mid, and modestly elevated but still far short of large_vehicle's real size in dense.
